...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
            ...
            &lt;!--[if gt IE 8]&gt;&lt;!--&gt; &lt;html class=&quot;no-js&quot; lang=&quot;en&quot;&gt; &lt;!--&lt;![endif]--&gt;
                ...
                <body>
                   ...
                   chromium.org/developers/how-tos/chrome-frame-getting-started -->
                    ...
                    <!-- KM: Hide sidebar on submissions page (except for admins) -->
                    <xsl:choose>
                        <xsl:when
                            test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request'][@qualifier='URI'] = 'submissions' and
                                  /dri:document/dri:meta/dri:userMeta/dri:metadata[@element='identifier'][@qualifier='firstName'] != 'Munin'">
                                <div class="col-xs-12 main-content">
                                            <xsl:apply-templates select="*[not(self::dri:options)]"/>
                        <!-- KM: Moved footer outside main container -->
                        <!--
    ...
    <xsl:template name="buildHead">
	    ...
	    <!-- KM: Add prefix Munin: to page titles -->
            <!-- Add the title in -->
...
<img src="https://www.ub.uit.no/matomo/matomo.php?idsite=1&amp;rec=1" style="border:0" alt="" />
<!-- End Matomo -->
</noscript>
        ...
        <header>
		    ...
		    <!-- KM: Added navbar-background -->
				...
				<xsl:attribute name="alt">
				    ...
				    <!-- KM: Query-string variable (to preserve http parameters) -->
				    <xsl:variable name="queryString">
				      <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request'][@qualifier='queryString']"/>
						...
						<!-- KM: Capitalize language string -->
                                                <a class="capitalize">
                                                    <xsl:attribute name="href">

						      <!-- KM: Preserve current queryString (except locale-attribute) -->
						      <xsl:choose>
							...
							<xsl:otherwise>
							  <xsl:choose>
							    <xsl:when test="contains($queryString, 'locale-attribute')">
							      <xsl:value-of select="$current-uri"/>
							      <xsl:value-of select="concat('?', substring($queryString, 1, string-length($queryString)-2), $locale)"/>
							    </xsl:when>
							    <xsl:otherwise>
							      <xsl:value-of select="$current-uri"/>
							      <xsl:value-of select="concat('?', $queryString, '&amp;locale-attribute=')" />
							      <xsl:value-of select="$locale"/>
							    </xsl:otherwise>
							  </xsl:choose>
							</xsl:otherwise>
						    ...
						    <!-- KM: Add flag next to language string -->
						    <img border="0" class="flag">
						      <xsl:attribute name="src">
							<xsl:value-of select="$theme-path"/>
							<xsl:text>images/lang-</xsl:text>
							<xsl:value-of select="$locale"/>
					    ...
					    <!--KM: Add My Submissions link here-->
                                            <li>
                                                <a href="{/dri:document/dri:meta/dri:pageMeta/
                            dri:metadata[@element='contextPath']}/submissions">
                    ...
                    <div class="navbar-header pull-right hidden-xs">
                        ...
                        <ul class="nav navbar-nav pull-left">
                            <xsl:choose>
                                <xsl:when test="/dri:document/dri:meta/dri:userMeta/@authenticated = 'yes'">
				  <!-- KM: Defining variables -->
				  <xsl:variable name="firstname" select="/dri:document/dri:meta/dri:userMeta/dri:metadata[@element='identifier' and @qualifier='firstName']"/>
				  <xsl:variable name="lastname" select="/dri:document/dri:meta/dri:userMeta/dri:metadata[@element='identifier' and @qualifier='lastName']"/>

                                    <li class="dropdown">
                                        <a id="user-dropdown-toggle" href="#" role="button" class="dropdown-toggle"
                                           data-toggle="dropdown">
					    <!-- KM: Reorganized the name output -->
                                            <span class="hidden-xs inline-block">
					      <!-- KM: Reorganized the name output -->
                                                <xsl:value-of select="util:shortenString($firstname, 19, 3)"/>
						<!--<xsl:text> </xsl:text>-->
						<br />
                                                <xsl:value-of select="util:shortenString($lastname, 19, 3)"/>
                                                <!--&#160;-->
                                                <!--<b class="caret"/>-->
                                            </span>
					    <span class="hidden-xs inline-block navbar-claret">
					      <b class="caret"/>
					    </span>
                                        </a>
                                        <ul class="dropdown-menu pull-right" role="menu"
    ...
    <xsl:template name="buildTrail">
        <div class="trail-wrapper hidden-print">
            <div class="container">
				...
				<!-- KM: Add a line below the breadcrumbs, and remove the background color -->
        ...
        <xsl:variable name="ccLogo">
             <xsl:choose>
                  <xsl:when test="starts-with($ccLicenseUri,
								  ...
								  starts-with($ccLicenseUri,
                                           'https://creativecommons.org/licenses/by/')">
                       <xsl:value-of select="'cc-by.png'" />
                  </xsl:when>
                  <xsl:when test="starts-with($ccLicenseUri,
                                          'http://creativecommons.org/licenses/by-sa/') or
								  starts-with($ccLicenseUri,
                  ...
                  <xsl:when test="starts-with($ccLicenseUri,
										  'http://creativecommons.org/publicdomain/zero/') or
								  starts-with($ccLicenseUri,
...
-->
				...
				<img class="logo-bottom" alt="UiT">
			  
				  <xsl:attribute name="src">
				    <xsl:value-of select="$theme-path"/>
				    <xsl:text>images/uit-logo-</xsl:text>
				    <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='currentLocale']"/>
				    <xsl:text>.png</xsl:text>
				  </xsl:attribute>
				</img>
...
-->
    ...
    <xsl:template match="dri:body">
        <div>
            ...
            <!-- KM: Add redirect if this is the login-chooser page -->
            <xsl:if test="dri:div[@n='login-chooser']">
              <xsl:call-template name="loginChooserRedirect"/>
            </xsl:if>
    ...
    <xsl:template name="languageSelection">
		...
		<xsl:variable name="queryString">
		  <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='request'][@qualifier='queryString']"/>
		    ...
		    <!-- KM: Capitalize language string -->
                    <span class="hidden-xs capitalize">
		      <!-- KM: Add flag to language selection -->
		      <img border="0" class="flag">
			<xsl:attribute name="src">
			  <xsl:value-of select="$theme-path"/>
			  <xsl:text>images/lang-</xsl:text>
			...
			<xsl:attribute name="alt">
			  <xsl:text>Velg spraak</xsl:text>
			    ...
			    <!-- KM: Capitalize language selection -->
                            <a class="capitalize">
                                <xsl:attribute name="href">
