...
-->
    ...
    <xsl:template name="itemSummaryList">
	  ...
	  <!-- KM: Remove thumbnails -->
	    ...
	    <!-- KM: Edit col style since the thumbnail col was removed -->
            <div class="col-sm-9 artifact-description">
			    ...
			    <xsl:if test="dri:list[@n=(concat($handle, ':dc.type'))]">
                    ...
                    <xsl:choose>
                        ...
                        <div class="abstract">
								<!-- KM: Show more of the abstract -->
                                <xsl:value-of select="util:shortenString(dri:list[@n=(concat($handle, ':dc.description.abstract'))]/dri:item[1], 400, 10)"/>
                        </div>
                    </xsl:when>
                    </xsl:choose>
                </div>
